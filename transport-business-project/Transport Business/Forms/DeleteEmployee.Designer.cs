namespace transport_business_project.Transport_Business.Forms
{
    partial class DeleteEmployee
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            button1 = new Button();
            button2 = new Button();
            label4 = new Label();
            label1 = new Label();
            cb_deleteEmployee = new ComboBox();
            SuspendLayout();
            // 
            // button1
            // 
            button1.Location = new Point(282, 277);
            button1.Name = "button1";
            button1.Size = new Size(75, 23);
            button1.TabIndex = 4;
            button1.Text = "Delete";
            button1.UseVisualStyleBackColor = true;
            // 
            // button2
            // 
            button2.Location = new Point(429, 277);
            button2.Name = "button2";
            button2.Size = new Size(75, 23);
            button2.TabIndex = 5;
            button2.Text = "Cancel";
            button2.UseVisualStyleBackColor = true;
            // 
            // label4
            // 
            label4.AutoSize = true;
            label4.Font = new Font("Showcard Gothic", 15.75F, FontStyle.Regular, GraphicsUnit.Point);
            label4.Location = new Point(282, 85);
            label4.Name = "label4";
            label4.Size = new Size(231, 27);
            label4.TabIndex = 10;
            label4.Text = "Delete an employee";
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.Location = new Point(285, 155);
            label1.Name = "label1";
            label1.Size = new Size(219, 15);
            label1.TabIndex = 11;
            label1.Text = "Choose an employee you want to delete";
            // 
            // cb_deleteEmployee
            // 
            cb_deleteEmployee.FormattingEnabled = true;
            cb_deleteEmployee.Location = new Point(309, 204);
            cb_deleteEmployee.Name = "cb_deleteEmployee";
            cb_deleteEmployee.Size = new Size(164, 23);
            cb_deleteEmployee.TabIndex = 12;
            // 
            // UpdateEmployee
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(800, 450);
            Controls.Add(cb_deleteEmployee);
            Controls.Add(label1);
            Controls.Add(label4);
            Controls.Add(button2);
            Controls.Add(button1);
            Name = "UpdateEmployee";
            Text = "UpdateEmployee";
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion
        private Button button1;
        private Button button2;
        private Label label4;
        private Label label1;
        private ComboBox cb_deleteEmployee;
    }
}